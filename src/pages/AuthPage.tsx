...
export default function AuthPage() {
  ...
  const handleSocialAuth = (method: "google" | "apple") => {
    // In production this triggers real OAuth. For prototype, simulate it.
    signIn({
      ...
      <div className="w-full max-w-md space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground text-sm">
            {mode === "signup" ? "Create your account to get started" : "Welcome back, hungry one"}
          </p>
        </div>

        <div className="bg-card border border-border rounded-2xl p-7 space-y-5 shadow-lg">
          {/* Social auth */}
          <div className="space-y-2.5">
            <button
              onClick={() => handleSocialAuth("google")}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-input bg-background text-sm font-medium text-foreground hover:bg-muted transition-colors"
            >
